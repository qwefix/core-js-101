...
function angleBetweenClockHands(date) {
  const minDegree = 6;
  const hourDegree = 30;
  const hourPerMinDegree = 0.5;

  const ourDate = new Date(date);
  const min = ourDate.getMinutes();
  const hour = ourDate.getUTCHours() % 12;
  let angle = Math.abs(min * minDegree - (hour * hourDegree) - hourPerMinDegree * min);
